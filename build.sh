...
# ─── Auto-setup ~/.hermes skills at boot (config.yaml written after key entry) ───
mkdir -p ~/.hermes/skills
if [ -d "/opt/rescue/config/skills" ]; then
    cp -rf /opt/rescue/config/skills/* ~/.hermes/skills/ 2>/dev/null || true
fi
...
ai_mode() {
    ...
    if [ -z "${DEEPSEEK_API_KEY:-}" ]; then
        ...
        # Write full config.yaml with real key now that we have it
        mkdir -p ~/.hermes/skills
        cat > ~/.hermes/config.yaml << HERMESCFG
# deepseekresc — Hermes Agent configuration
...
models:
  deepseek:
    ...
    deepseek-v4-pro:
      provider: deepseek
HERMESCFG
        # Copy skills if not already there
        if [ -d "/opt/rescue/config/skills" ]; then
            cp -rf /opt/rescue/config/skills/* ~/.hermes/skills/ 2>/dev/null || true
        fi
    fi
