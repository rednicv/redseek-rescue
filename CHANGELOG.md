...
### Added
- **Professional Presentation** — Added `PRESENTATION.md` with a comprehensive technical overview of the project (architecture, components, AI agent flow, security measures, build pipeline). Generated by Claude Opus 4.6 via Antigravity AI Pro bridge.
- **Version badge** in README updated to v1.8.0.
- **Presentation badge** in README linking to PRESENTATION.md.

### Fixed
- **API key injection at boot** — config.yaml now properly written with the real DeepSeek key when the user pastes it at first boot (instead of only saving the placeholder).
